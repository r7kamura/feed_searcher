...
describe FeedSearcher do
  describe ".search" do
    ...
    context "when the specified resource has feed MIME type and parsable as XML" do
      before do
        stub_request(:get, "http://example.com/").to_return(
      ...
      it "returns itself if it is a feed url" do
        FeedSearcher.search("http://example.com/").should == %w[
          http://example.com/
        ]
      end
    ...
    context "when the specified resource has feed extension" do
      before do
